...
describe('List 엔티티는', () => {
  ...
  it('노출 조건은 변경 가능하며, 허용되지 않는 값으로 설정할 경우에는 에러를 발생시켜야 한다', () => {
    ...
    list.displayType = DisplayType.DELETED

    expect(list.displayType).toEqual(DisplayType.DELETED)

    const changeWrongDisplayType = () => (list.displayType = 'wrong' as unknown as DisplayType)
    
    expect(changeWrongDisplayType).toThrowError('type must be one of DisplayType')
  })
})